...
struct mm_struct {
	struct {
		...
		/*
		 * Per-mm scheduling-hint area: a list of grow-on-demand
		 * segments (each a reserved user VMA plus the kernel-owned
		 * pages backing it), packed into 64-byte slots and shared by
		 * all threads of this process. NULL = not enabled. Lifetime is
		 * bound to the mm; freed once in __mmput() after exit_mmap().
		 * See kernel/sched/hint.c.
		 */
